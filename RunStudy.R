...
# Project Name: BigDataRetailProject
...
setwd("/Users/anuradhavasudeva/")
#local_directory <- "~INSEADAnalytics/CourseSessions/Sessions45"
local_directory <- paste(getwd(),"BigDataRetailProject/", sep="/")
...
segmentation_attributes_used = c(2, 6, 9, 12, 14) 

# Please ENTER then original raw attributes to use for the profiling of the segments (the "profiling attributes")
# Please use numbers, not column names! e.g. c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8
profile_attributes_used = c(2, 6, 9, 12, 14) # for boats use c(2:82), for Mall_Visits use c(2:9)

# Please ENTER the number of clusters to eventually use for this report
numb_clusters_used = 8 # for boats possibly use 5, for Mall_Visits use 3
...
if (start_local_webapp){
  ...
  StoreSales <- read.csv(paste(local_directory, "data/StoreSales.csv", sep = "/"), sep=";", dec=",") # this contains only the matrix ProjectData
  StoreSales=data.matrix(StoreSales) # this file needs to be converted to "numeric"....
